...
const DEFAULT_HELP = [
  ...
  '',
  '  pull',
  '    Sync ~/.claude/ from the shared repo (settings, symlinks, sessions).',
  '  pull --dry-run',
  '    Run lock + git pull, then preview every mutation without writing.',
  '',
  '  push',
  '    Rebase, run safety checks (gitleaks, gitlinks, allow-list), commit, push.',
  '  push --dry-run',
  '    Run pre-checks (rebase, gitleaks probe, gitlink scan) and preview remap,',
  '    without staging or pushing.',
  '',
  '  diff',
  '    Offline preview of what `pull` would change against local repo state.',
  '    No git pull, no lock acquired.',
  '',
  '  init',
  '    Scaffold an empty ~/claude-nomad/ repo (shared/, hosts/, path-map).',
  '  init --snapshot',
  '    Overlay the current ~/.claude/ into shared/ as the initial seed.',
  '',
  '  doctor',
  '    Read-only health check (symlinks, host file, path-map, gitleaks, gitlinks).',
  '  doctor --resume-cmd <id>',
  '    Print `cd <abspath> && claude --resume <id>` for a session id from',
  '    ~/.claude/projects/.',
  '',
