...
describe('reportRebaseState', () => {
  ...
  it('emits a FAIL line and sets exitCode=1 on a mid-rebase repo', async () => {
    ...
    const { reportRebaseState } = await import('./git-state.ts');
    ...
    const { renderDoctor } = await import('../format.ts');
    renderDoctor([sec]);
    const out = joinedLog(env.logSpy);
    expect(out).toContain(failGlyph);
...
describe('reportRebaseClean', () => {
  ...
  it('emits a WARN line when the repo has uncommitted changes', async () => {
    ...
    const { reportRebaseClean } = await import('./git-state.ts');
    const { section, renderDoctor } = await import('../format.ts');
    const sec = section('Repository');
...
describe('reportOrphanedAutostash WARN', () => {
  ...
  it('emits a WARN line when an orphaned autostash is present and does NOT set exitCode', async () => {
    ...
    const { reportOrphanedAutostash } = await import('./git-state.ts');
    const { section, renderDoctor } = await import('../format.ts');
    const sec = section('Repository');
  ...
  it('emits both FAIL (unmerged-index) and WARN (autostash) independently in the same run', async () => {
    ...
    addUnmergedIndex(tmp);
    const { reportRebaseState, reportOrphanedAutostash } = await import('./git-state.ts');
    const { section, renderDoctor } = await import('../format.ts');
    const sec = section('Repository');
    reportRebaseState(sec);
    reportOrphanedAutostash(sec);
...
describe('reportGitIdentity', () => {
  ...
  it('emits a PASS row (okGlyph) when user.name and user.email are both set', async () => {
    ...
    const { reportGitIdentity } = await import('./git-state.ts');
    const { section, renderDoctor } = await import('../format.ts');
    const sec = section('Repository');
...
describe('reportTrackedDeniedShared', () => {
  ...
  async function run(): Promise<string> {
    const { reportTrackedDeniedShared } = await import('./git-state.ts');
    const { section, renderDoctor } = await import('../format.ts');
    const sec = section('Repository');
