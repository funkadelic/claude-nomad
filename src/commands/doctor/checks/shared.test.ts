...
type CheckSharedMemoryModule = typeof import('./shared.memory.ts');
...
describe('reportCheckShared buildScanTree unit tests (gitleaks mocked)', () => {
  ...
  afterEach(() => {
    ...
    vi.doUnmock('./shared.scan.ts');
    vi.doUnmock('./shared.memory.ts');
    restoreEnv(snapshot, testHome);
  });
  ...
  function mockGitleaksAndCaptureScanArgs(): () => number {
    ...
    vi.doMock('./shared.scan.ts', async (importOriginal) => {
      const actual =
        // eslint-disable-next-line @typescript-eslint/consistent-type-imports
        await importOriginal<typeof import('./shared.scan.ts')>();
      return {
        ...actual,
  ...
  it('recovers from a local-preview scan throw: FAIL row + exitCode=1, and reportCommittedMemory still runs', async () => {
    ...
    vi.doMock('./shared.memory.ts', async (importOriginal) => {
