...
function makeAdoptEnv(): Env {
  ...
  return {
    ...
    repoHome,
    claudeHome,
    // Mirrors `lockFilePath` in utils.lockfile.ts, which is private to that
    // module. Every refusal raised under the lock must leave this path gone; a
    // `process.exit` on such a path would skip `cmdAdopt`'s `finally` and
    // strand the lockfile.
...
describe('cmdAdopt (precondition matrix)', () => {
  ...
  it.skipIf(isWin)(
    'already-a-symlink branch: no shared/<name> at all is not adopted either',
    async () => {
      // The fourth state of the same branch. The local link resolves (to
      // somewhere outside the repo), so it reaches the already-a-symlink arm,
      // but the repo carries no counterpart, so the link cannot be pointing
      // into the sync repo and "already adopted" would be the same false
      // success the dangling and unreadable arms already removed.
      addSharedDir(env, 'my-dir');
      const elsewhere = join(env.testHome, 'somewhere-else');
      mkdirSync(elsewhere, { recursive: true });
      symlinkSync(elsewhere, join(env.claudeHome, 'my-dir'));
      ...
      expect(out).toContain('there is no shared/my-dir in the repo, so it is not adopted');
      expect(out).not.toContain('already adopted');
      // Reporting, not writing: the exit code and the index stay untouched,
      // matching the other two unhealthy arms of this same branch.
...
describe('cmdAdopt (happy path and move sequence)', () => {
  ...
  it('prints the exact ADOPT_PUSH_HINT literal', async () => {
    ...
    const mod = await import('./adopt.ts');
...
describe('cmdAdopt never-sync refusal', () => {
  ...
  it.skipIf(isWin)(
    ...
    async () => {
      ...
      const { cmdAdopt, ADOPT_PUSH_HINT } = await import('./adopt.ts');
      expect(() => cmdAdopt('my-tools')).not.toThrow();
...
describe('cmdAdopt widened never-sync boundary', () => {
  ...
  it('the staged sessions path from that adopt survives the push allow-list gate', async () => {
    ...
    const { enforceAllowList } = await import('./push/allowlist.ts');
...
describe('cmdAdopt win32 copy-back branch', () => {
  ...
  // symlink arm.
  it.skipIf(isWin)(
    'win32: refuses a dangling shared/<name> at exit 1 when the host entry is absent',
    async () => {
      addSharedDir(env, 'my-dir');
      symlinkSync(
        join(env.repoHome, 'shared', 'no-such-target'),
  ...
  it.skipIf(isWin)(
    'win32: refuses a dangling shared/<name> at exit 1 when a real local copy is present',
    async () => {
      addSharedDir(env, 'my-dir');
      const linkPath = join(env.claudeHome, 'my-dir');
      mkdirSync(linkPath, { recursive: true });
...
describe('isDirectChildOf', () => {
  ...
  it('accepts a direct child', async () => {
    const { isDirectChildOf } = await import('./adopt.recover.ts');
...
describe('removeAdoptSource containment bound', () => {
  ...
  it('refuses a path outside ~/.claude/ rather than removing it', async () => {
    ...
    const { removeAdoptSource } = await import('./adopt.recover.ts');
...
describe('cmdAdopt backup failure', () => {
  ...
  it('reports a snapshot write failure as its own error rather than a crash report', async () => {
    ...
    // report for what is an expected failure. The guard now lives inside the
    // real backupBeforeWrite/backupUnder, so the mock targets node:fs cpSync
    // rather than the helper, which is what makes this exercise the real
    // guard end to end instead of a stand-in for it.
    addSharedDir(env, 'my-tools');
    const linkPath = join(env.claudeHome, 'my-tools');
    mkdirSync(linkPath, { recursive: true });
    writeFileSync(join(linkPath, 'tool.sh'), '#!/bin/sh\necho hi\n');
