...
describe('resolveTomlConfig (overlay merge logic)', () => {
  ...
  it('overlay present but bundled base absent -> path null, tempPath null, no throw', async () => {
    // Overlay present, repo toml absent. packageRoot's own package.json walk
    // must still resolve (a broken install is a different failure mode than a
    // merely-absent bundled toml), but the bundled toml itself is absent.
    vi.doMock('node:fs', async (importOriginal) => {
      const actual = await importOriginal<typeof fsModule>();
      return {
        ...actual,
        existsSync: vi.fn(
          (p: unknown) =>
            String(p).endsWith('.gitleaks.overlay.toml') || String(p).endsWith('package.json'),
        ),
      };
    });
    const { resolveTomlConfig } = await import('./gitleaks.config.ts');
...
describe('resolveTomlPath (two-tier toml lookup)', () => {
  ...
  it('returns the bundled path when repo copy is absent but bundled exists', async () => {
    ...
    vi.doMock('node:fs', async (importOriginal) => {
      ...
      return {
        ...actual,
        // REPO_HOME toml absent; everything else (packageRoot's package.json
        // walk, then the bundled toml itself) present.
        existsSync: vi.fn((p: unknown) => String(p) !== join('/fake/repo', '.gitleaks.toml')),
      };
    });
    const { resolveTomlPath } = await import('./gitleaks.config.ts');
  ...
  it('returns null when neither repo copy nor bundled copy exists', async () => {
    ...
    vi.doMock('node:fs', async (importOriginal) => {
      const actual = await importOriginal<typeof fsModule>();
      return { ...actual, existsSync: vi.fn((p: unknown) => String(p).endsWith('package.json')) };
    });
    const { resolveTomlPath } = await import('./gitleaks.config.ts');
