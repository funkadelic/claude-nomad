/**
 * Argv parser for `nomad sync [--dry-run]`.
 *
 * `--dry-run` is the only accepted flag; sync deliberately does not accept
 * `--force-remote` (wedge recovery stays on the explicit `nomad pull
 * --force-remote` command) or any of the push resolution flags
 * (`--redact-all`, `--allow`, `--allow-all`, `--full-scan`). Advanced cases
 * route to the low-level `push`/`pull` commands.
 *
