 ...
 * that temp tree. On findings it renders the same push-style
 * `buildSessionAwareFatal` message and sets `process.exitCode = 1`. On a
 * clean scan it logs a short no-leaks line.
 *
...
/**
 * Classify scan findings and render the appropriate output line(s). On a null
 * report (scan crash) emits a scan-failed error line and sets exitCode 1. On an
 * empty findings array logs a clean no-leaks line. On non-empty findings calls
 * `buildSessionAwareFatal` and emits the FATAL body, setting exitCode 1.
 *
 * @param findings - Output of `scanStagedTree`, or `null` on scan crash.
 */
function reportPreviewFindings(findings: Finding[] | null): void {
  if (findings === null) {
    fail('dry-run gitleaks preview: scan failed, no parseable report');
    process.exitCode = 1;
    return;
  }
  if (findings.length === 0) {
    log('dry-run: gitleaks preview clean, no leaks');
    return;
  }
  const { bySession, other } = partitionFindings(findings);
  fail(buildSessionAwareFatal(bySession, other));
  process.exitCode = 1;
}
 ...
 * is never written.
 *
 * On findings: emits the same `buildSessionAwareFatal` body as a real push and
 * sets `process.exitCode = 1`. On a clean scan: logs a short no-leaks line,
 * exitCode unchanged. On a scan crash (`scanStagedTree` returns `null`): emits
 * a scan-failed error line and sets `process.exitCode = 1`.
 *
 * @param map - Parsed `path-map.json` (already in scope from `cmdPush`).
 */
export function previewPushLeaks(map: PathMap): void {
  ...
  try {
    ...
    if (sessionCount + extrasCount === 0) {
      log('dry-run: nothing to scan, no leaks');
      return;
    }
    ...
    try {
      ...
      fail('dry-run gitleaks preview: scan error (git or gitleaks not on PATH)');
      process.exitCode = 1;
      return;
    }
    reportPreviewFindings(findings);
  } finally {
