...
- **One shared setup, per-machine exceptions.** Your own skills, slash commands, rules, and your
  ...
  Code's `skills/synced/` folder stay behind (see `SHARED_LINKS` in `src/core/config.ts` and
  `src/sync/skills-sync.ts`). Claude Code fills that folder with your claude.ai account skills on
  each machine. Settings merge a shared base with a per-host override, so one machine can run a
  different model or MCP URL without forking the rest. GSD-owned hook entries (scripts whose
  basename starts with `gsd-`) are filtered out of the generated `~/.claude/settings.json` during
  pull and stripped from `shared/settings.base.json` on the next push; GSD reinstalls the correct
  per-host hook set itself. A non-gsd hook you add to your live settings syncs normally via
