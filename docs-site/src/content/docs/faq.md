...
checklist. It also deletes the records nomad kept about this machine. After it runs it prints a
short manual-remainder checklist: uninstall the CLI, drop the `NOMAD_HOST` / `NOMAD_REPO` env vars,
and optionally delete the sync repo and nomad's cache folder. Preview first with `nomad eject --dry-run`. See the
