...
and telemetry. The authoritative list is the `NEVER_SYNC` set in `src/core/config.never-sync.ts`, and
...
checklist. It also deletes the records nomad keeps about this machine. After it runs it prints a
short manual-remainder checklist: uninstall the CLI, drop the `NOMAD_HOST` / `NOMAD_REPO` env vars,
and optionally delete the sync repo and nomad's cache folder. Preview first with
`nomad eject --dry-run`. See the
...
start with `gsd-` and is not one of Claude Code's own `synced`, `.trash` or `.staging` folders)
still sync, but as a filtered copy rather than a symlink. The `gsd-*` skills are excluded from both
push and pull; gsd reinstalls them per host via npm.
