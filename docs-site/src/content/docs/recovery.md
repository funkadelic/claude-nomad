...
- **Allow** appends the finding's fingerprint to `.gitleaksignore` at the repo root, but only once
  ...
  The embedded path also uses the scanning host's separators, so an entry recorded on native Windows
  (backslashes) does not suppress the same finding scanned from macOS, Linux, or WSL2, and vice
  versa; allow it again from the other host when that happens.
...
skills and Claude Code's own `skills/synced/`, `skills/.trash/` and `skills/.staging/` folders are
excluded). Skill content is executable (`SKILL.md`, helper scripts in `references/` or `scripts/`,
dotfiles), so it is a plausible place for a pasted API key or token to end up. A gitleaks finding in
a skill file, like a memory note, has no session id, so it gets the same treatment:
