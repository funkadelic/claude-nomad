...
repo before they fetch, so an unpublished edit is never overwritten. A file you delete is handled the
same way: it is removed from the repo by the next pull too, exactly as deleting inside a symlinked
directory already removes it on macOS or Linux. Enabling Developer Mode does not change this; copies
are used on every native Windows host either way.
:::
...
- **Shared config is copied, not symlinked.** On macOS and Linux, files like `CLAUDE.md` and your
  ...
  this machine, so a repo file this machine has never synced is never touched. The one command that
  deliberately takes the repo's version is `nomad pull --force-remote`, which is what that flag is
  for; the copy it replaces is snapshotted to the backup dir first. This is the same behavior
