...
| `--check-shared`    | Read-only gitleaks preflight: stages the session transcripts a `push` would publish into a temp tree and scans them, failing (`✗`, exit 1) per affected session. Skips with a `⚠︎` when gitleaks is not on PATH. Also runs a separate, WARN-only advisory (`⚠︎`, exit code untouched) over `memory/*.md` files already committed to the sync repo, pointing at the push-recovery Redact step; a latent memory secret never fails this check. See [Recovery flows](/claude-nomad/recovery/). |
