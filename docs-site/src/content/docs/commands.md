...
| Flag               | Description                                                                                                                                                                                                                                                        |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--dry-run`        | Run pre-push safety checks (gitleaks probe, rebase, remap preview, gitlink scan, allow-list) and a read-only gitleaks leak preview over a throwaway temp copy of the sessions, extras, and non-gsd user skills this host would stage. Exits with code 5 if a leak is found (the same leak-blocked code a real push uses, so a scripted `$? == 5` pre-flight buckets it the same way), or code 1 if the scan itself cannot run. Writes nothing to `~/.claude/` and commits/pushes nothing, but the rebase above updates the sync repo (`~/claude-nomad/`) first.    |
