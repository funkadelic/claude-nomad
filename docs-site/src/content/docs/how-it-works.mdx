...
- **The win32 mirror is filtered against this same never-synced set.** Copying your local edits
  ...
  the repo rather than through `~/.claude/`, the next `nomad pull` removes or restores it and warns
  once, naming the file. Because the set is not only secrets, an ordinary directory inside one of
  your shared names can collide with it (a `sessions` or `tasks` subfolder, for example) and
  silently stop mirroring; only an exact directory name collides, so a file named `tasks.md` is
  unaffected. Nothing in this gate can fail a pull outright: a hit is a warning and a revert, never
  an error.
