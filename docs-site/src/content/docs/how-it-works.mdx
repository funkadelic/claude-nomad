...
- **The `.claude/` extra is filtered against the _full_ never-synced set, not just that subset.**
  ...
  ephemeral or host-local name on push (`settings.local.json`, `projects/`, `shell-snapshots/`,
  `sessions/`, `statsig/`, `telemetry/`, `todos/`, and the rest of `NEVER_SYNC`), leaving only your
  config (`settings.json`, `hooks/`, `agents/`, `skills/`, `commands/`, `rules/`). So syncing
  `.claude/` carries your project's Claude setup across machines without ever leaking session
  transcripts or per-host secrets. The same boundary is enforced a second time at the push gate as a
  backstop.
