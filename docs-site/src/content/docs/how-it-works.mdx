...
- **The `.claude/` extra is filtered against the _full_ never-synced set, not just that subset.**
  ...
  `telemetry/`, `todos/`, ...), leaving only your config (`settings.json`, `skills/`, `commands/`,
  `rules/`). So syncing `.claude/` carries your project's Claude setup across machines without ever
