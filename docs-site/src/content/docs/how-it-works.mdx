...
| **Per-project extras** | Whitelisted dirs like `.planning/` or a project's own `<repo>/.claude/`, or a root file like `CLAUDE.md` | Opt-in via the `extras` field in `path-map.json`; overlaid to/from `shared/extras/<logical>/` (additive, so files that exist only in the repo or only on a host survive). |
...
- **A pull keeps each machine's own per-host files.** Because those never-synced names are stripped
  ...
  updates your shared `settings.json`, `hooks/`, and the rest from the repo without ever
  overwriting or deleting the local-only settings that machine keeps to itself.
...
competes with git, and because a nomad pull overlays the repo extra onto your working tree
...
as everything else. The copy is an overlay, not a mirror: it adds and updates files but never
deletes, so anything that lives only in the repo (a `.planning/` file another host pushed, say)
survives a push from a host that does not have it.

On `nomad pull`, the reverse copy runs after `git pull --rebase`, and just before it overwrites
your working tree a divergence check compares the incoming content against your local copy and
prints a per-file WARN naming anything that differs. The pull is an overlay too, so local-only
files (work in progress not yet pushed) are left in place rather than wiped. The one exception is
genuine upstream deletions: for `.planning`, nomad diffs the repo's pre- and post-rebase HEADs and
removes locally only the files that the rebase actually deleted upstream, so a teammate's `git rm`
still propagates without a blind mirror wiping everything else.
