...
encoded path is the absolute project path with every character that isn't a letter or digit replaced
by `-` (and very long paths shortened with a short hash on the end). So the same logical project ends
up in different directories on each host.
