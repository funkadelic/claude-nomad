...
- **The win32 mirror is filtered against this same never-synced set.** Copying your local edits
  ...
  subfolder, for example) and silently stop mirroring; only an exact directory name collides, so a
  file named `tasks.md` is unaffected. Nothing in this gate can fail a pull outright: a hit is a
