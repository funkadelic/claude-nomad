...
- **Credentials stay blocked even inside synced extras.** A strict subset of the never-synced set
  ...
  file dropped under a synced `.planning/` or `.claude/` directory can never ride through the extras
  gate. For `.claude/` specifically, the push copy filters these names out before staging, so
  opting `.claude/` in never leaks per-host secrets (for example, `settings.local.json` is skipped
  while `settings.json` and files under `hooks/` are copied normally).
