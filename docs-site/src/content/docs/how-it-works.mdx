...
shared it no longer has the original. That one is left alone as well, with a warning naming the
entry, on `nomad push` and on the pull-side mirror alike. Nomad does not repair it for you: remove
it from the sync repo, or restore what it points at, by hand. A later step in the same pull reads
