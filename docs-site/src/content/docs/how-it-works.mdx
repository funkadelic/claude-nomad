...
never going to write to the repo. A later step in the same pull reads that file too, and it no
longer stops there: it warns as well, naming the file, saying whether the file is gone or may be
part-updated, and pointing at the backup copy when it made one, then skips that one name and
finishes the rest of the pull. A file you delete from inside one of these directories is mirrored
