...
- **The sync repo is a trust boundary for pulling hosts.** Content pulled from the repo (session
  ...
  variant cannot slip past on macOS, and it also ignores trailing dots and spaces, so a spelling
  like `.env.` is refused exactly like `.env`. A secret dropped under a synced `.planning/`
