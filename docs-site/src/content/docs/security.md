...
- **The sync repo is a trust boundary for pulling hosts.** Content pulled from the repo (session
  ...
  secret dropped under a synced `.planning/` directory cannot ride through. The `.claude/` extra is
  filtered more strictly still: it is checked against the full `NEVER_SYNC` set plus `projects/`, so
  opting a project's `.claude/` directory in strips session transcripts and every per-host state
  directory on push, leaving only config (`settings.json`, `hooks/`, `agents/`, `skills/`,
  `commands/`, `rules/`). But the broader principle holds: a host trusts whatever the repo contains.
